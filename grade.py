...
if __name__ == "__main__":
    ...
    files = [
        ...
        if isfile(join(DATA_PATH, f)) and f.startswith("in")
    ]

    success_count = 0
    db_file = join(DATA_PATH, "olympics.db")

    for file in sorted(files):
        file_in = join(DATA_PATH, file)
        num = int(file[2:-4])
        file_ans = join(DATA_PATH, f"ans{num}.csv")
        ...
        p = subprocess.Popen(
            ["Rscript", PROGRAM, db_file, file_out],
        ...
        if success:
            ...
            if len(ans) != len(out):
                message.append(
                    f"Line count mismatch: should be {len(ans)}, got {len(out)}"
                )
                success = False
            else:
                for i in range(len(ans)):
                    if out[i] != ans[i]:
                        message.append(
                            f"Line {i} mismatch: should be '{ans[i]}', get '{out[i]}'"
                        )
                        success = False
                        break
        ...
        if success:
            ...
            if os.isatty(1):
                ...
                if len(stderr) > 0:
                    message.append(
                        f'Your program exited with: {stderr.decode("utf-8")}'
                    )
